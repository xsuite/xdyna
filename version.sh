...
sed -i "s/\(__version__ =\).*/\1 '"${new_ver}"'/"         xdyna/general.py
sed -i "s/\(assert __version__ ==\).*/\1 '"${new_ver}"'/" tests/test_version.py
echo "Committing version change..."
git reset
git add pyproject.toml xdyna/general.py tests/test_version.py
...
# Draft release
gh release create v${new_ver} --draft --generate-notes --title "Xdyna release ${new_ver}" --verify-tag
